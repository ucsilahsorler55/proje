...
class Club(db.Model):
    ...
    memberships = db.relationship(
        "ClubMembership", back_populates="club", lazy=True, cascade="all, delete-orphan"
    )
    events = db.relationship(
        "Event", back_populates="club", lazy=True, cascade="all, delete-orphan"
    )
    announcements = db.relationship(
        "Announcement", back_populates="club", lazy=True, cascade="all, delete-orphan"
    )
    ...
    def to_dict(self, include_members=False):
        data = {
            ...
            "founding_date": (
                self.founding_date.isoformat() if self.founding_date else None
            ),
            "status": self.status,
        ...
        if include_members:
            data["member_count"] = sum(
                1 for m in self.memberships if m.status == "active"
            )
        return data
...
class ClubMembership(db.Model):
    ...
    role = db.Column(
        db.String(50), default="member"
    )  # member, president, vice_president, treasurer
    status = db.Column(
        db.Enum("pending", "active", "inactive", name="membership_status"),
        default="active",
    )
...
class ClubApplication(db.Model):
    ...
    applicant = db.relationship(
        "User", foreign_keys=[applicant_id], backref="club_applications"
    )
    reviewer = db.relationship("User", foreign_keys=[reviewed_by])
    founders = db.relationship(
        "ClubApplicationFounder",
...
class EventInvitation(db.Model):
    ...
    user = db.relationship("User", foreign_keys=[user_id], backref="event_invitations")
    ...
    def to_dict(self):
        return {
            ...
            "event_date": (
                ...
                if self.event and self.event.event_date
                else None
            ),
            "event_location": self.event.location if self.event else None,
            "club_name": (
                self.event.club.name if self.event and self.event.club else None
            ),
            "user_id": self.user_id,
            "user_name": (
                f"{self.user.first_name} {self.user.last_name}" if self.user else None
            ...
            "invited_by_name": (
                f"{self.inviter.first_name} {self.inviter.last_name}"
                if self.inviter
                else None
            ),
            "status": self.status,
            "created_at": self.created_at.isoformat() if self.created_at else None,
            "invitation_type": "event",  # Davet türünü belirtmek için
        }
